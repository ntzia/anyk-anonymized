...
import algorithms.cycles.SimpleCycle_YannSorting_Iterator;
...
public class Equijoin {
    ...
    static SimpleCycle_Equijoin_Query cycle_query = null;
    static int max_k = -1;
    ...
    public static void run(String query_type) {
        if (algorithm.equals("Count")) {
            ...
            } else if (query_type.equals("cycle")) {
                DP_Path_Equijoin_Instance instance = new DP_Path_Equijoin_Instance(path_query);
                System.out.println("Number_of_Results = " + instance.count_solutions());
            }
            // Error
            else {
                System.err.println("Algorithm not currently supported for cycle queries");
                System.exit(1);
            }
            return;
        }
        if (algorithm.equals("Boolean")) {
            ...
            for (int k = 1; k <= max_k; k++) {
                ...
                {
                    if (k == 1)
                        measurements.add_k(new Tuple(new double[]{0}, 0, null));
                    break;
                }
                else
            ...
            if (query_type.equals("cycle")) {
                SimpleCycle_YannSorting_Iterator yann = new SimpleCycle_YannSorting_Iterator(cycle_query);
                Tuple t;
                for (int k = 1; k <= max_k; k++) {
                    t = yann.get_next();
                    if (t == null)
            ...
            if (query_type.equals("tree") || query_type.equals("cycle")) {
                System.err.println("Algorithm not currently supported for tree queries");
                System.exit(1);
            }
            DP_Path_Equijoin_Instance instance = new DP_Path_Equijoin_Instance(path_query);
            ...
            if (query_type.equals("path")) {
                ...
                SimpleCycle_Anyk_Iterator iter = new SimpleCycle_Anyk_Iterator(cycle_query, algorithm, null);
                Tuple solution;
    ...
    public static void main(String args[]) {
        ...
        if (query.equals("path")) {
            ...
            cycle_query = new SimpleCycle_Equijoin_Query(database);
        } else {
            if (cmd.hasOption("relationNo"))
                l = Integer.parseInt(cmd.getOptionValue("relationNo"));
            else {
