...
/**
 * Implementation of Anyk-PartMemoized for T-DP, a ranked enumeration algorithm
 * that improves upon {@link algorithms.paths.TDP_Part}
 * by adding certain memoization.
 * In particular, we store the order that a prefix explores a subspace and then
 * reuse that order for other prefixes too.
 * The data structures needed to for memoization are contained in
 * {@link entities.paths.TDP_SuffixMemoizer}.
 * One such object is pointed to from each
 * {@link entities.paths.TDP_DecisionSet}.
 * 
...
public abstract class TDP_PartPlus extends TDP_Anyk_Iterator {
	...
	/**
	 * Stores for every stage whether it is an independence point for the tree. This
	 * means that given a node of that stage,
	 * the prefixes and suffixes are independent. With a BFS ordering, this
	 * corresponds to the last stage of every level.
	 */
	...
	public TDP_PartPlus(TDP_Problem_Instance inst, Configuration conf) {
		...
		for (int sg = 1; sg < this.stages_no - 1; sg += 1) {
			if (instance.get_parent_stage(sg) != instance.get_parent_stage(sg + 1))
				independence_points.add(true);
			else
				independence_points.add(false);
		}
		// The last stage is always an independence point
		independence_points.add(true);

		// Initialize the global PQ with an empty prefix (that contains only the
		// starting node)
		...
		// The prefix we start with contains only the best decision to go from
		// starting_node to stage 1
		// That way, we guarantee that for top-2 we start taking successor solutions
		// from stage 1
		if (instance.starting_node.get_opt_cost() != Double.POSITIVE_INFINITY) // Corner case: if no path can reach the
																				// terminal node, leave the pq empty
		{
	...
	/**
	 * For this algorithm, the expansion, successor-taking and following steps are
	 * combined.
	 * As a result, it is difficult to return a popped result as soon as possible.
	 * To emulate the behavior of other any-k algorithms, return the top-1 result
	 * immediately (known from bottom-up DP)
	...
	public TDP_Solution get_next_helper() {
		...
		if (popped_solution instanceof TDP_Prefix_Solution) {
			...
			// Convert the prefix to a list so that we can look up the parent nodes in each
			// step
			List<TDP_State_Node> node_list = curr_prefix.solutionToNodes_strict_order();

			// This loop expands the prefix until complete or until a top-1 suffix is found
			// in the sorted lists
			while (curr_prefix.length < instance.stages_no - 1) {
				...
				if (sorted_suffixes != null) {
					...
					TDP_Prefix_Solution_Follower expanded_solution = new TDP_Prefix_Solution_Follower(curr_prefix,
							sorted_suffixes.get(0), 0);
					...
					// Create the corresponding suffix by looking up the best stored suffix from
					// that non-leading prefix
			...
			if (curr_suffix == null)
				curr_suffix = new TDP_Suffix_Solution(curr_prefix.latest_decision);
			else
			...
			while (curr_prefix != null) {
				...
				if (this.independence_points.get(curr_prefix.length + 1)) {
					...
					if (curr_decisionset.sorted_suffixes == null) {
						// System.out.println("Stage " + curr_prefix.length + " : Recording " +
						// curr_prefix + " as leading prefix");
					...
					// As we traverse the solution backwards, create a suffix of the decisions so
					// that we can store it in the sorted order
	...
	/**
	 * Makes a given prefix "follow" the leading prefix that ends on the same
	 * decision set.
	 * Specifically, we find the next best suffix from the sorted list of suffixes
	 * discovered by the leading prefix.
	 * If it hasn't been computed yet, we make the given prefix wait until it has (a
	 * subscriber).
	 * 
	 * @param follower A prefix solution that is a follower.
	 * @param decs     The decision set where the data structures related to the
	 *                 prefix are stored.
	 */
	private void follow_leading_prefix(TDP_Prefix_Solution_Follower follower, TDP_DecisionSet decs) {
		...
		if (decs.sorted_suffixes.size() > next_rank) {
			...
			// System.out.println("The next best suffix from " + follower.prefix + " is " +
			// decs.sorted_suffixes.get(next_rank)));
			global_pq.add(
					new TDP_Prefix_Solution_Follower(follower.prefix, decs.sorted_suffixes.get(next_rank), next_rank));
		} else {
			// Subscribe
			// System.out.println("Prefix " + follower.prefix + " is subscribing for suffix
			// with rank " + next_rank);
	...
	/**
	 * Records a suffix as the next best option in the sorted list of suffixes at
	 * the end of the given prefix.
	 * 
	...
	private void record_suffixes(TDP_Prefix_Solution prefix, TDP_Suffix_Solution suffix, TDP_DecisionSet decs) {
		// System.out.println("Stage " + prefix.length + " : Adding " + suffix + " to
		// sorted suffix list with rank " + decs.sorted_suffixes.size());
		assert !list_contains_reference(decs.sorted_suffixes, suffix) : "Duplicate suffix in sorted list";
		assert decs.sorted_suffixes.isEmpty() ||
				Double.compare(decs.sorted_suffixes.get(decs.sorted_suffixes.size() - 1).get_cost() - 0.0000001,
						suffix.get_cost()) <= 0
		...
		for (TDP_Prefix_Solution_Follower subscriber : decs.subscribers) {
			// System.out.println("Subscriber " + subscriber.prefix + " receives rank-" +
			// subscriber.rank_of_suffix + " suffix: " + suffix);
			assert subscriber.rank_of_suffix == decs.sorted_suffixes.size() - 1
					: "Subscriber was waiting for suffix of different rank";
	...
	/**
	 * Expands the prefix solution into a full solution by following an optimal
	...
	protected TDP_Prefix_Solution expand(TDP_Prefix_Solution pref) {
		...
		// assert String.format("%.4f",
		// current.future_cost).equals(String.format("%.4f", current.get_cost())); //
	 ...
	 * 
	 * @param decisions The set of decisions whose data structures will be
	 *                  initialized.
	 */
	public abstract void initialize_partial_order(TDP_DecisionSet decisions);

	/**
	 * Find the successors of a given decision among the other decisions in the
	 * {@link entities.trees.TDP_DecisionSet}.
	 ...
	 * @return TDP_Decision The successor decisions according to the (partial)
	 *         order.
	 */
	...
	private static void timing_experiment_all() {
		...
		for (int i = 0; i < times_to_repeat; i++) {
			...
			// for (int j = 1; j < l - 1; j++) q.add_to_tree_wConjunction(db.get(j), j, j -
			// 1, Arrays.asList(new Join_Predicate("E", 1, 0, null)));
			// q.add_to_tree_wConjunction(db.get(l - 1), l - 1, l - 3, Arrays.asList(new
			// Join_Predicate("E", 0, 0, null)));

			// Path Query
			q.add_to_tree_wConjunction(db.get(0), 0, -1, null);
			for (int j = 1; j < l; j++)
				q.add_to_tree_wConjunction(db.get(j), j, j - 1, Arrays.asList(new Join_Predicate("E", 1, 0, null)));
	...
	public static void main(String args[]) {
		...
		// TDP_Thetajoin_Instance instance = new TDP_Thetajoin_Instance(example_query,
		// null);
		// instance.bottom_up();
		// // Print the cost of the optimal solution
		// //System.out.println("Optimal cost = " +
		// instance.starting_node.get_opt_cost());
		...
		// Tree_ThetaJoin_Query q = new Tree_ThetaJoin_Query();
		// q.add_to_tree_wConjunction(db.get(0), 0, -1, null);
		// for (int j = 1; j < 3; j++) q.add_to_tree_wConjunction(db.get(j), j, j - 1,
		// Arrays.asList(new Join_Predicate("E", 1, 0, null)));
		// q.add_to_tree_wConjunction(db.get(3), 3, 0, Arrays.asList(new
		// Join_Predicate("E", 1, 0, null)));
