...
*/
public class SimpleCycle_YannSorting_Iterator
{
    /** 
    ...
    */
     ...
     */
	public SimpleCycle_YannSorting_Iterator(SimpleCycle_Equijoin_Query query)
    {
        ...
        */
        ...
        Tuple first_solution;

        // Initialize the priority queue that will compare solutions of the different acyclic problems
        this.pq = new PriorityQueue<Pair<Tuple, YannakakisSorting>>(new Comparator<Pair<Tuple, YannakakisSorting>>()
        {
            public int compare(Pair<Tuple, YannakakisSorting> p1, Pair<Tuple, YannakakisSorting> p2) 
            {
        ...
        {
            YannakakisSorting iter = new YannakakisSorting(subquery);
            //iter.set_name(name.toString());
            first_solution = iter.get_next();
            if (first_solution != null) pq.add(new Pair<Tuple, YannakakisSorting>(first_solution, iter));
    ...
    public Tuple get_next()
    {
        ...
        if (last_iterator != null)
        {
            ...
            Tuple next_sol = last_iterator.get_next();
            if (next_sol != null) pq.add(new Pair<Tuple, YannakakisSorting>(next_sol, last_iterator));            
        }
        // Now pick the best one from the pq and return it
        if (!pq.isEmpty())
        {
            Pair<Tuple, YannakakisSorting> best_pair = pq.poll();
            Tuple tup = best_pair.getValue0();
            tup.set_relation(output);
