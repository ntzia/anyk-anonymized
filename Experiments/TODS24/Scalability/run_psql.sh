...
do
    ...
    do
        ...
        do
            ...
            # Generate sql query
            ./"generate_sql_${q}.py" `pwd`/$DATA_PATH ${l} ${k} psql
            # Run the query
            psql $USER < sql_queries/${q}_l${l}_k${k}_psql.sql 2>> "${OUT_PATH}${infile}_${q}_l${l}_k${k}_psql.out" | grep -A 9999 "QUERY PLAN" >> "${OUT_PATH}${infile}_${q}_l${l}_k${k}_psql.out"
            # Delete the csv files
            rm ${DATA_PATH}Edges.csv
        done
        echo "Done with $graph, $q, psql, run ${i}"
    done
done
...
do
    ...
    do
        ...
        do
            ...
            ./"generate_sql_${q}.py" `pwd`/$DATA_PATH ${l} ${k} psql
            # Run the query
            psql $USER < sql_queries/${q}_l${l}_k${k}_psql.sql 2>> "${OUT_PATH}synthetic_${q}_n${n}_l${l}_d${d}_psql.out" | grep -A 9999 "QUERY PLAN" >> "${OUT_PATH}synthetic_${q}_n${n}_l${l}_d${d}_psql.out"
            # Delete the csv files
            for j in $(seq 1 ${l});
            do
            ...
            echo "Done with synthetic $q, n=$n, d=$d, psql, run $i"
        done
    done
done
...
do
    ...
    do
        ...
        do
            ...
            ./"generate_sql_${q}.py" `pwd`/$DATA_PATH ${l} ${k} psql
            # Run the query
            psql $USER < sql_queries/${q}_l${l}_k${k}_psql.sql 2>> "${OUT_PATH}synthetic_gauss_${q}_n${n}_l${l}_std${std_dev}_psql.out" | grep -A 9999 "QUERY PLAN" >> "${OUT_PATH}synthetic_gauss_${q}_n${n}_l${l}_std${std_dev}_psql.out"
            # Delete the csv files
            for j in $(seq 1 ${l});
            do
                rm "${DATA_PATH}R${j}.csv"
            ...
            echo "Done with synthetic $q, n=$n, std=$std_dev, psql, run $i"
        done
    done
done
...
do
    ...
    do
        ...
        do
            ...
            ./in_to_csv.py "$DATA_PATH${infile}.in"
            # Generate sql query
            ./"generate_sql_${q}.py" `pwd`/$DATA_PATH ${l} ${k} psql ${graph^^}
            # Run the query
            psql $USER < sql_queries/${q}_l${l}_k${k}_psql.sql 2>> "${OUT_PATH}${infile}_${q}_l${l}_k${k}_psql.out" | grep -A 9999 "QUERY PLAN" >> "${OUT_PATH}${infile}_${q}_l${l}_k${k}_psql.out"
            # Delete the csv files
            rm ${DATA_PATH}Edges.csv
