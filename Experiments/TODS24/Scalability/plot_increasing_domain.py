...
parser.add_argument('-d', nargs='+', dest="dom_list", default=[], help="list of max domain values (x-axis)")
...
for i in range(len(algorithms)):
	...
	for dom in dom_list:
		...
		# Read file
		try:
			fp1 = open(inFileName + "_d" + str(dom) + "_" + alg + ".out")
		except IOError:
...
for i in range(len(algorithms)):
	...
	if x_labels_list:
		...
		ax.plot(dom_list, times[alg], label=alg_label, marker = markers[i], markersize = markersizes[i], fillstyle=fillstyles[i])
...
if k_to_plot == 1000:
	ax.set(xlabel="domain size", ylabel="TT($10^3$) sec")
	ax.xaxis.label.set_size(20)
else:
	ax.set(xlabel="domain size", ylabel="TT(" + str(k_to_plot) + ") sec")
