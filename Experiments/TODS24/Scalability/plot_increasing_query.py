...
parser.add_argument('-l', nargs='+', dest="l_list", default=[], help="list of query lengths")
	...
	for l in l_list:
		...
		try:
			if alg == "psql" or alg == "sysx" or alg == "duckdb":
				fp1 = open(inFileName + "_" + query + "_l" + str(l) + "_k" + str(k_to_plot) + "_" + alg + ".out")
			else:
				fp1 = open(inFileName + "_" + query + "_l" + str(l) + "_" +  alg + ".out")
		except IOError:
...
if k_to_plot == 1000:
	ax.set(xlabel=r"$\ell$ (#relations in join)", ylabel="TT($10^3$) sec")
	ax.xaxis.label.set_size(20)
else:
	ax.set(xlabel=r"$\ell$ (#relations in join)", ylabel="TT(" + str(k_to_plot) + ") sec")
